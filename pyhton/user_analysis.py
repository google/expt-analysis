...
android-randomMail, android-search, ios-randomSocialApp, windows-randomMail
...
CountItemsPerSlice(df=df, itemCols=['user_id', 'prod'], sliceCols=['country', 'date'], newColName='slice_num_items')
'''
...
# for each slice, if usage occurrs for some item but not for others for that slice
...
def CalcItemPenet(
  ...
  perItemDf2 = perItemDf[sliceCols + usageCols + ['item_count_in_slice', 'penetration']].copy()
...
# percent of items in slice who has usage (eg app usage) aka penetration
...
# eg if itemCols = [user_id, date], it will add
...
# its slower because it complates the data grid
...
def CalcPerItemMetrics_withDistr(
  ...
  # to damping the variation: investigate
...
## slower method, with golbal filling, not recommended usually
...
Mark(itemMetricsDf1, 'fast method, but only means')
Mark(itemMetricsDf2, 'slower method but more metrics, bySlice filling, only existing items')
Mark(itemMetricsDf3, 'slower method but more metrics, bySlice filling, all items combin within slice')
Mark(itemMetricsDf4, 'slower method but more metrics, global filling, all items combin globally, matching slice with user_id only')
'''

## penetration calculated in per item metrics have the caveat of a shifting denominator
# to remedy this we could consider for example: #(user, date, prod) / # users in a period
...
# so for example for one user, we find out the number of dates where user used usage:facebbok
...
Mark(
    res4[:20],
    "global, all item/slice combin: this is what you shouldn't do normally")
...
Mark(
    res5[:20],
    "global, all item combin, all items combin globally, matching slice with user_id only")
'''
...
def CompareUsageSlices_withDistr(
  ...
  if regDictPost is not None:
    ...
    dfMetrics = dfMetrics.reset_index()
    if unitCols is not None:
      dfMetrics_avgItemNum = dfMetrics_avgItemNum[BuildRegexInd(
          df=dfMetrics_avgItemNum, regDict=regDictPost)]
      dfMetrics_avgItemNum = dfMetrics_avgItemNum.reset_index()
  ...
  for valueCol2 in ['usage_count_mean', valueCol + '_total_mean', 'penetration']:
    dfMetrics[valueCol2 + '_ci_lower'] = dfMetrics[valueCol2 + '_ci_lower'].map(lambda x: max(x, 0.0))
    pltDict[valueCol2 + '_ci'] = PlotCIWrt(
      ...
      logScale=False,
      lowerLim=None,
      pltTitle=pltTitlePre + valueCol2 + '_' + itemColsPltTitle + ' CI for mean',
      figSize=figSize)
  ...
  if unitCols is not None:
    pltDict['avg_itemWithUsage_num_ci'] = PlotCIWrt(
      ...
      logScale=False,
      lowerLim=None,
      pltTitle=pltTitlePre + 'item_with_usage_count' + '_' + itemColsPltTitle + ' CI for mean',
      figSize=figSize)
...
df.loc[len(df)] = ['US', '0', '2017-04-12', 'SLIDES']
df.loc[len(df)] = ['US', '0', '2017-04-12', 'PHOTOS']
df.loc[len(df)] = ['US', '0', '2017-04-12', 'SLIDES']
df.loc[len(df)] = ['US', '0', '2017-04-12', 'SLIDES']
df.loc[len(df)] = ['US', '0', '2017-04-12', 'SHEETS']
df.loc[len(df)] = ['US', '2', '2017-04-12', 'SLIDES']
df.loc[len(df)] = ['US', '2', '2017-04-12', 'SHEETS']
df.loc[len(df)] = ['US', '2', '2017-04-12', 'DOCS']
df.loc[len(df)] = ['US', '2', '2017-04-12', 'DOCS']

def PropRow(row):
  out = float(row['prod'] == 'DOCS')
  return(out)
def PropDf(df):
  return(df['prod'] == 'DOCS')
...
def CheckIndCols_satisfyMultipleProp(
  ...
  for i in range(len(propDfList)):
    ...
    res = CheckIndCols_satisfyProp_atleastOnce(
        df=df, indCols=indCols, PropDf=PropDf, PropRow=None, propColName=propColName)
    ...
    if i == 0:
      ...
      aggDf['prop_multiple_binary'] = aggDf['prop_multiple_binary'] * aggDf[propColName + '_binary']
...
'''
df = GenUsageDf_forTesting()
TestObsCounts_acrossSlices(
    df=df, countCol='user_id', sliceCols=['expt'], popSize=100)
'''
